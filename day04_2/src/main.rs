...
fn get_from_string(s:&str,start:&str,end:&str)->String
{
    let s  = s.to_string();
    let st = s.find(start);
    let en = s.find(end);
...
fn get_seconds(s:String) -> i32 {
    let lr: Vec<_> = s.split(" ").collect();
    let ms: Vec<_> = lr[1].split(":").collect();
    let mins = ms[0].parse::<i32>().unwrap();
    let secs = ms[1].parse::<i32>().unwrap();
    60*mins + secs
}


impl Guard {
    fn new(i:i32,a:Action,t:String)->Self {
        Guard{
            id      : i,
            action  : a,
            time    : t.clone(),
            time_min: get_seconds(t),
        }
    }   

    fn from_string(s:String)->Self {
        let mut act : Action = Action::Sleep;
        let time = get_from_string(&s[..],"[","]");
        let mut id = -1;

        if s.find("begins shift")!=None 
        {
            let ids = get_from_string(&s[..],"Guard #"," begins");
            id = ids.parse::<i32>().unwrap();
            act = Action::GuardChange(id);
        }
        else
        if s.find("wakes up")!=None 
        {
            act = Action::WakeUp;
        }
...
fn solve(data:Vec<&str>)->i32 {
    ...
    for d in data {
        v.push(Guard::from_string(d.to_string()));
    }

    v.sort_by(|a, b| (&a.time).cmp(&b.time) );

    for i in 0..v.len() {
        if v[i].id!=-1 { id = v[i].id }
        v[i].id = id;
    }

    let mut sleep_time = HashMap::new();
    let mut sleep_time_all = 0;

    for s in v.clone() {
        match s.action {
            Action::Sleep => {
                sleep_time_all = s.time_min;
            },
            Action::WakeUp => {
                sleep_time.insert(s.id,sleep_time.get(&s.id).unwrap_or(&0i32) + s.time_min - sleep_time_all);
            },
            _ => {}
        }
    }
        ...
        for s in v.clone() {
            if s.id==id {
                match s.action {
                    ...
                    Action::WakeUp => {
                        for i in sleep_time_all..s.time_min {
...
#[test]
fn test1() {
    let s = get_from_string("[1518-11-01 00:00] Guard #10 begins shift","Guard #"," begins");
    assert_eq!(s,"10");
}
